...
class PacsDataGrey(data_utils.Dataset):
    def __init__(self, path, domain_list=None, mode='train'):
        ...
        self.to_grey = transforms.Grayscale(num_output_channels=3)
    ...
    def get_imgs_and_labels(self, file_name, domain_path):
        ...
        for i, img_path in enumerate(img_paths):
            ...
            img_list.append(self.to_tensor(self.to_grey(img)))
...
if __name__ == "__main__":
    ...
    train_loader = data_utils.DataLoader(
        PacsDataGrey('./kfold/', domain_list=domain_list_train, mode='train'),
        batch_size=128,
        shuffle=True)
    ...
    for i, (x, y, d) in enumerate(train_loader):
        ...
        if i == 0:
            n = min(x.size(0), 36)
            save_image(x[:n].cpu(),
                       'pacs_train_grey.png', nrow=6)

    print(y_array, d_array)
    print('\n')

    train_loader = data_utils.DataLoader(
        PacsDataGrey('./kfold/', domain_list=domain_list_train, mode='val'),
        batch_size=128,
        shuffle=True)
    ...
    for i, (x, y, d) in enumerate(train_loader):
        ...
        if i == 0:
            n = min(x.size(0), 36)
            save_image(x[:n].cpu(),
                       'pacs_val_grey.png', nrow=6)

    print(y_array, d_array)
    print('\n')

    train_loader = data_utils.DataLoader(
        PacsDataGrey('./kfold/', domain_list=domain_list_train, mode='test'),
        batch_size=128,
        shuffle=True)
