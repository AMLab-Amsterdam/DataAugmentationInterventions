...
for restart in range(flags.n_restarts):
  ...
  def make_environment(images, labels, e):
    ...
    images_zero = torch.zeros((images.shape[0], 14, 14))
    ...
    images = torch.stack([images, images, images_zero.byte()], dim=1)
  ...
  random_color = torchvision.transforms.ColorJitter(brightness=0.6, contrast=10.0, saturation=50.0, hue=0.5)
  ...
  for i in range(2):
      for j in range(envs[i]['images'].shape[0]):
          image = envs[i]['images'][j]
          transformed_image = to_tensor(random_color(to_pil(image)))
          envs[i]['images'][j] = transformed_image

  # Push everything to cuda
  for i in range(3):
      envs[i]['images'] = envs[i]['images'].cuda()

  # Check augmentations
  save_image(envs[0]['images'][:64].cpu(),
             'color_mnist_random.png', nrow=8)
